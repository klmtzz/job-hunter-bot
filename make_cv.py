...
def render_project_item(proj: dict) -> str:
    ...
    if proj.get("bullets"):
        ...
        for b in proj["bullets"]:
            b_formatted = re.sub(r"\*\*(.+?)\*\*", r"<strong>\1</strong>", b)
            b_formatted = re.sub(r"`(.+?)`", r"<code>\1</code>", b_formatted)
            items.append(f"<li>{b_formatted}</li>")
    ...
    if proj.get("link"):
        ...
        if clean_url.startswith("http"):
            href = clean_url
            label = clean_url.replace("https://", "").replace("http://", "")
        else:
            href = f"https://{clean_url}"
            label = clean_url
        link_html = f'\n            <a href="{href}" class="project-link" target="_blank"><svg viewBox="0 0 24 24"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg>{label}</a>'
        
    stack_pills = ""
    if proj.get("stack"):
        pills = [s.strip().replace("`", "") for s in proj["stack"].split("|") if s.strip()]
        stack_pills = "".join(f'<span class="proj-badge">{p}</span>' for p in pills)
        
    return f'''
        <div class="project-card">
            <div class="project-header">
                <span class="project-title">{proj["title"]}</span>
                <div class="project-badges">{stack_pills}</div>
            </div>{bullets_html}
            {link_html}
        </div>'''
...
def render_section_to_html(section: dict) -> str:
    ...
    if any(k in title_lower for k in ["tech stack", "технологии", "stack"]):
        ...
        for line in lines:
            ...
            is_title = line_str.startswith("###") or (line_str.startswith("**") and not line_str.startswith(">") and "Стек" not in line_str and "github" not in line_str)
            if is_title:
                if current_project:
                    html += render_project_item(current_project)
                title_text = re.sub(r"^(?:###\s*)?(?:\d+\.\s*)?", "", line_str).replace("**", "").strip()
                current_project = {
                    "title": title_text,
                    "stack": "",
                    "bullets": [],
                ...
                if "Стек:" in line_str:
                    m_stack = re.search(r"Стек:?\s*(.+)$", line_str, re.IGNORECASE)
                    if m_stack:
                        current_project["stack"] = m_stack.group(1).strip()
                elif "github.com" in line_str.lower():
                    m_link = re.search(r"\[(.*?)\]\((https?://github\.com/[^\)]+)\)", line_str) or re.search(r"(github\.com/[^\s\)\>]+)", line_str)
                    if m_link:
                        current_project["link"] = m_link.group(2) if m_link.lastindex and m_link.lastindex >= 2 else m_link.group(1)
                elif line_str.startswith("> *") or line_str.startswith("* ") or line_str.startswith("- ") or line_str.startswith("> -") or line_str.startswith(">"):
                    clean = re.sub(r"^>\s*[\*\-•]?\s*", "", line_str).strip()
                    if clean and not clean.lower().startswith("**github:**") and not clean.lower().startswith("стек:"):
                        current_project["bullets"].append(clean)
                else:
